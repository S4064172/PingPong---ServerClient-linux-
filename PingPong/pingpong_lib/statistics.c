...
/*** TO BE DONE START ***/
	...
	double last_comodo=(double)(last->tv_sec)*1000;
	last_comodo+=(double)(last->tv_nsec)/1000000;
	double previous_comodo=(double)(previous->tv_sec)*1000;
	previous_comodo+=(double)(previous->tv_nsec)/1000000;
	return (double)(last_comodo -previous_comodo);
		      ...
		      double rtt[repeats], int msg_sz, double resolution)
{
	...
	qsort(rtt, (size_t)repeats, sizeof(double), double_cmp);//double_cmp efettua l'offset del rtt in base ai dati passati alla qsort
