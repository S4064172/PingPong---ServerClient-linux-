...
double do_ping(size_t msg_size, int msg_no, char message[msg_size], int ping_socket, double timeout)
{
	...
	//byte ricevuti e mandati 
	ssize_t recv_bytes, sent_bytes;
	//strutture per il calcolo dell'rtt 
...
/*** TO BE DONE START ***/
		//mando il messagio dal server
		if((sent_bytes=send(ping_socket,message,msg_size,0))<0)
			fail_errno("Error: send udp");
/*** TO BE DONE END ***/
...
/*** TO BE DONE END ***/
		...
		//se non mi ha bloccato il sistema operativo e la recv fallisce allora...
...
/*** TO BE DONE START ***/
	if ((gai_rv = getaddrinfo(pong_addr, pong_port , &gai_hints, &pong_addrinfo))!= 0) 
		fail_errno("ERROR: getaddrinfo udp");
/*** TO BE DONE END ***/
